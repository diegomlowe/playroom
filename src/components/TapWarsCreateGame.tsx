...
import { setGames } from '@/lib/collections/games';
...
const MNY_TIERS: BuyInTier[] = [
  {
    ...
    secondPrize: parseFloat((MNY_CASUAL * 4 * 0.2250).toFixed(6)),
  ...
  },
  {
    ...
    secondPrize: parseFloat((MNY_STANDARD * 4 * 0.2250).toFixed(6)),
  ...
  },
  {
    ...
    secondPrize: parseFloat((MNY_HIGH * 4 * 0.2250).toFixed(6)),
    fee: parseFloat((MNY_HIGH * 4 * 0.025).toFixed(6)),
  },
];

function PrizeCard({ tier, onClick, currency }: { tier: BuyInTier; onClick: () => void; currency: Currency }) {
...
export const TapWarsCreateGame: React.FC = () => {
  ...
  const handleCreate = async (tier: BuyInTier) => {
    ...
    setCreating(true);
    try {
      const gameId = `tw-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      ...
      if (success) {
        toast.success(`${tier.name} game created!`);
        navigate(`/games/${gameId}`);
      } else {
        toast.error('Failed to create game. Check policy permissions.');
      }
    } catch (err) {
      ...
      <div className="relative min-h-screen flex flex-col overflow-hidden">
        ...
        <header className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2">
          ...
          <div className="text-right">
            <h1 className="text-xl font-black tracking-widest gradient-text" style={{ fontFamily: "'Orbitron', sans-serif" }}>
              TAPWARS
            </h1>
        ...
        <section className="relative z-10 px-4 pt-8 pb-6 text-center">
          <h2 className="text-3xl sm:text-4xl font-black leading-tight mb-2 gradient-text" style={{ fontFamily: "'Orbitron', sans-serif" }}>
            New Game
          </h2>
          <p className="text-muted-foreground text-sm text-center leading-relaxed">
            Pick your buy-in tier. Higher stakes, bigger prizes.
          </p>
        </section>
          ...
          {creating ? (
            <div className="flex items-center justify-center min-h-[200px]">
              <div
                ...
                style={{ fontFamily: "'Orbitron', sans-serif", color: 'hsl(280 100% 65%)' }}
              >
                CREATING...
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 max-w-md mx-auto">
              {tiers.map((tier) => (
                <PrizeCard key={`${currency}-${tier.id}`} tier={tier} onClick={() => handleCreate(tier)} currency={currency} />
              ))}
            </div>
          )}
        </section>
...
export default TapWarsCreateGame;
